...
class ParkingApiException implements Exception {
  ...
  @override
  String toString() => 'ParkingApiException(message: $message)';
}

class ParkingApiService {
  final Dio _dio;

  ParkingApiService({Dio? dio}) : _dio = dio ?? AppHttpClient.instance;

  Future<List<Map<String, dynamic>>> fetchParkings() async {
    try {
      final Response<dynamic> response = await _dio.get<dynamic>(
        ApiConstants.publicParkingsPath,
        options: Options(
      ...
      if (statusCode != 200) {
        throw ParkingApiException(_extractMessage(payload));
      }
      ...
      throw ParkingApiException(_mapDioError(error));
    }
  }
  ...
  String _mapDioError(DioException error) {
    switch (error.type) {
      ...
      case DioExceptionType.receiveTimeout:
        return 'Delai depasse pour charger les parkings.';
      case DioExceptionType.connectionError:
        return 'Impossible de contacter le serveur des parkings.';
  ...
  String _extractMessage(Map<String, dynamic> payload) {
    ...
    return 'Impossible de charger les parkings.';
  }
}
